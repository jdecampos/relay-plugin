---
description: Traite les PRD Tikett en attente (.ticket/) — implémente, commit, PR draft, callback.
argument-hint: "[ref d'un PRD précis, optionnel]"
---

Tu livres des PRD produits par Tikett. La source de vérité est le dossier
`.ticket/` à la racine de CE repo. Ton abonnement Claude Code fait le travail —
il n'y a aucun agent côté serveur Tikett.

## 1. Se synchroniser avec le dépôt (indispensable)

Tikett pousse les PRD (`.ticket/index.json` + `.ticket/prd/*.md`) sur la **branche
...
- Mets-la à jour : si tu es dessus, `git pull --ff-only`. Sinon bascule dessus
  ...
  partiront les branches `tikett/<id>`.
- Si après ça `.ticket/index.json` n'existe toujours pas, c'est qu'aucune
  livraison n'a encore été lancée depuis Tikett : dis-le et arrête-toi.

## 2. Charger la file

- Lis `.ticket/config.json` (`tikettUrl`, `projectId`) et `.ticket/index.json`.
...
2. Crée une branche dédiée **depuis la branche de base à jour** :
   `git switch <base> && git switch -c tikett/<id>` (si `tikett/<id>` existe déjà,
...
6. Commit sur la branche : `git add -A` puis un commit clair
   (`git commit -m "feat: <title> (Tikett <id>)"`). Récupère le SHA
   (`git rev-parse HEAD`), réécris-le dans `commit` de `index.json`, et amende le
   commit pour inclure ce dernier ajustement.
7. Push : `git push -u origin tikett/<id>`.
8. Ouvre une **PR draft** si `gh` est disponible :
   `gh pr create --draft --title "<title>" --body "Livraison du PRD Tikett <id>."`.
   Sinon, indique l'URL de comparaison à l'utilisateur.
9. **Notifie Tikett** : exécute
   `bash "${CLAUDE_PLUGIN_ROOT}/bin/tikett-callback.sh" <id> <sha>`.
...
`failed` : `bash "${CLAUDE_PLUGIN_ROOT}/bin/tikett-callback.sh" <id> <sha> failed`.

Règles Tikett : jamais d'auto-merge (la PR reste en draft, un humain valide),
